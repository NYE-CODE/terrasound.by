...
router = APIRouter(prefix=ADMIN_V2_PREFIX, tags=["admin-content"], dependencies=ADMIN_ROUTER_DEPENDENCIES)

DEFAULT_LIST_LIMIT = 100
MAX_LIST_LIMIT = 500


@router.get("/installation-services", response_model=PaginatedOut[InstallationServiceOut])
def list_installation_services_admin_v2(
    db: Annotated[Session, Depends(get_db)],
    limit: int = Query(default=DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(default=0, ge=0),
) -> PaginatedOut[InstallationServiceOut]:
    query = db.query(InstallationService).order_by(InstallationService.sort_order)
    total = query.count()
    items = query.offset(offset).limit(limit).all()
    data = [InstallationServiceOut.model_validate(item) for item in items]
    return paginated(data, total=total, limit=limit, offset=offset)


@router.post("/installation-services", response_model=InstallationServiceOut, status_code=201)
def create_installation_service_admin_v2(
...
@router.patch("/installation-services/{item_id}", response_model=InstallationServiceOut)
def update_installation_service_admin_v2(
    item_id: str,
...
@router.delete("/installation-services/{item_id}", status_code=204)
def delete_installation_service_admin_v2(
    item_id: str,
    db: Annotated[Session, Depends(get_db)],
) -> None:
    item = db.query(InstallationService).filter(InstallationService.id == item_id).first()
...
def list_brands_admin_v2(
    db: Annotated[Session, Depends(get_db)],
    limit: int = Query(default=DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(default=0, ge=0),
) -> PaginatedOut[BrandOut]:
    query = db.query(Brand).order_by(Brand.sort_order, Brand.name)
    total = query.count()
    items = query.offset(offset).limit(limit).all()
    data = [BrandOut.model_validate(item) for item in items]
    return paginated(data, total=total, limit=limit, offset=offset)


@router.post("/brands", response_model=BrandOut, status_code=201)
def create_brand_admin_v2(
    payload: BrandCreate,
    db: Annotated[Session, Depends(get_db)],
) -> BrandOut:
...
@router.get("/blog-posts", response_model=PaginatedOut[BlogPostOut])
def list_blog_admin_v2(
    db: Annotated[Session, Depends(get_db)],
    limit: int = Query(default=DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(default=0, ge=0),
) -> PaginatedOut[BlogPostOut]:
    query = db.query(BlogPost).order_by(BlogPost.created_at.desc())
    total = query.count()
    items = query.offset(offset).limit(limit).all()
    data = [BlogPostOut.model_validate(item) for item in items]
    return paginated(data, total=total, limit=limit, offset=offset)


@router.post("/blog-posts", response_model=BlogPostOut, status_code=201)
def create_blog_admin_v2(
...
@router.get("/categories", response_model=PaginatedOut[CategoryAdminOut])
def list_categories_admin_v2(
    db: Annotated[Session, Depends(get_db)],
    limit: int = Query(default=DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(default=0, ge=0),
) -> PaginatedOut[CategoryAdminOut]:
    items = db.query(Category).order_by(Category.sort_order, Category.name).all()
    counts = _product_counts_by_category(db)
    all_data = [_category_admin_out(item, counts.get(item.id, 0)) for item in items]
    total = len(all_data)
...
@router.get("/portfolio-works", response_model=PaginatedOut[PortfolioWorkAdminOut])
def list_portfolio_admin_v2(
    db: Annotated[Session, Depends(get_db)],
    limit: int = Query(default=DEFAULT_LIST_LIMIT, ge=1, le=MAX_LIST_LIMIT),
    offset: int = Query(default=0, ge=0),
) -> PaginatedOut[PortfolioWorkAdminOut]:
    query = db.query(PortfolioWork).order_by(PortfolioWork.sort_order, PortfolioWork.title)
    total = query.count()
    items = query.offset(offset).limit(limit).all()
    data = [PortfolioWorkAdminOut.model_validate(item) for item in items]
    return paginated(data, total=total, limit=limit, offset=offset)


@router.post("/portfolio-works", response_model=PortfolioWorkAdminOut, status_code=201)
def create_portfolio_work_admin_v2(
...
def update_portfolio_work_admin_v2(
    item_id: str,
...
def delete_portfolio_work_admin_v2(
    item_id: str,
    db: Annotated[Session, Depends(get_db)],
) -> None:
    item = db.query(PortfolioWork).filter(PortfolioWork.id == item_id).first()
